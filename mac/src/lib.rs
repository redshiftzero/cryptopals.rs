...
pub fn gen_md4_keyed_mac(key: &[u8], message: &[u8]) -> [u8; 16] {
    let mut hasher = Md4::new();
    let content = [key, message].concat();
    hasher.update(&content);

    let result = hasher.finalize(); // Returns GenericArray
...
pub fn verify_md4_keyed_mac(key: &[u8], message: &[u8], mac: &[u8]) -> bool {
    let test_mac = gen_md4_keyed_mac(key, message);
...
mod tests {
    ...
    #[test]
    fn test_keyed_mac_and_verification_md4() {
        // Needed for challenge 30
        let key = "key".as_bytes();
        let message = "secret secret".as_bytes();
        let mac = gen_md4_keyed_mac(&key, &message);

        let result = verify_md4_keyed_mac(&key, &message, &mac);
        assert_eq!(result, true);

        let not_key = "not da key".as_bytes();
        let result = verify_md4_keyed_mac(&not_key, &message, &mac);
        assert_eq!(result, false);
    }
    ...
    #[test]
    fn test_break_keyed_mac_using_length_extension_md4() {
        // This solves Set 4, Challenge 30. Same as test_break_keyed_mac_using_length_extension
        // buuuut using MD4 instead of SHA-1. The main difference in MD4 is that it uses little-endian
        // byte order, the hash is 16 bytes instead of 20, and there are only 4 states.
        let key = "key".as_bytes();
        let message =
            "comment1=cooking%20MCs;userdata=foo;comment2=%20like%20a%20pound%20of%20bacon"
                .as_bytes();
        ...
        for (n, chunk) in mac.chunks_exact(4).enumerate() {
            state[n] = u32::from_le_bytes(slice_to_arr(&chunk));
        }
        ...
        for key_len in 3..12 {
            let mut hasher = Md4::new();
            hasher.set_state(state);
            // If you found this on Le Goog and are debugging your MD4 length extension solution,
            // try the following debug step: ensure that the states are getting set correctly
            // on the attacker's Md4 struct (compare with the original struct)
            println!("{:?}", hasher.get_state());
            ...
            let result = verify_md4_keyed_mac(&key, &forged_message, &forged_tag);
